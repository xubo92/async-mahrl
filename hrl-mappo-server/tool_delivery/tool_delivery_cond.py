...
class toolDeliveryCond(ObjSearchDelivery_v4):
    def __init__(self, *args, **kwargs):
        super(toolDeliveryCond, self).__init__(*args, **kwargs)
        self.agent_names = ["turtlebot1", "turtlebot2", "fetchbot"]
        self.agt_avail_acts = []
        self.agt_avail_acts.append([1] * 3 * 3 * 6)
  
        self.observation_space = [[13 + 13 + 4, []]*1]
        self.share_observation_space = [[13 + 13 + 4, []]*1]
        self.action_space = [Discrete(3*3*6)]
    ...
    def step(self, actions):
        local_obs, shared_obs, reward = [], [], None
        dones = [False]
        infos = []
        terminate = False
        cur_actions_done = np.array([0,0,0])

        decoupled_actions = self.act_table[actions[0][0]]
        while(not terminate and not np.any(cur_actions_done)):
            cur_actions, observations, rewards, terminate, cur_actions_done, termi_info = self.OSDv4_parent.step(decoupled_actions, debug=False)
            reward = rewards
            self.basic_steps += 1

        local_obs.append(np.concatenate(observations))
        shared_obs.append(np.concatenate(observations))
        reward = [[reward[0]]] * 1
...
if __name__ == "__main__":
    ...
    while (i <= 150):
        ...
        cat_obs = np.concatenate(observations)
        i += 1
