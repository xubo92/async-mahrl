...
class captureTargetSeparate(object):
    def __init__(self, max_cycles=35, continuous_actions=True, **kwargs):
        ...
        if kwargs["all_args"].use_centralized_V:
            ...
            self.share_observation_space = [[8+8]]*2
            self.action_space = [spaces.Box(low = -1.0, high = 1.0, shape=(2,), dtype=np.float32)] * 2
        else:
            self.observation_space = [[8], [8]]
            self.share_observation_space = [[8], [8]]
            self.action_space = [spaces.Box(low = -1.0, high = 1.0, shape=(2,), dtype=np.float32)] * 2
    ...
    def step(self, action):
        ...
        local_obs = cur_obs.copy()
        share_obs = cur_obs.copy()
        reward = [np.mean(cur_reward), np.mean(cur_reward)]
    ...
    def reset(self):
        local_obs, shared_obs = None, None
        self.meta_env.reset()
        cur_obs = []
        for j in range(2):
            cur_obs.append(np.concatenate((self.meta_env.world.agents[j].state.p_vel, \
