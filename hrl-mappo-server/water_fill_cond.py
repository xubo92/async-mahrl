"""
This is used for testing a more complex action space (49-dim)
"""
import sys
...
class waterFillCond(object):
    ...
    def __init__(self, args, scene="FloorPlan_Train3_1"):
        ...
        if self.args.ac_sp == "full":
            ...
            self.action_list = []
            for i, locobot_act in enumerate(["go2cup2fill", "go2mug2fill", "go2btl2fill", "MoveAhead", "MoveBack", "RotateLeft", "RotateRight"]):
                for j, drone_act in enumerate(["go2cup", "go2mug", "go2btl", "MoveAhead", "MoveBack", "RotateLeft", "RotateRight"]):
        ...
        self.max_ep_steps = 200                        # Maximum number of Option steps, default 200
        self.basic_steps  = 0
        ...
        self.agt_avail_acts = []
        if self.args.conditioned and self.args.ac_sp == "combi" and self.args.mode=="multi":
            self.agt_avail_acts.append([1]*49)
        else:
        ...
        if self.args.conditioned and self.args.ac_sp == "combi" and self.args.mode=="multi":
            self.observation_space = [[1030, []]*1]
            self.share_observation_space = [[1030, []]*1]
    ...
    def reset(self):
        ...
        # when use combinational, joint actions, compress [o1,o2] by their channel dim
        if self.args.conditioned and self.args.ac_sp == "combi" and self.args.mode=="multi":
            np_imvecs = []
            for i, obs_img in enumerate(rgb_imgs):
                ...
                np_imvecs.append(np_imvec)
            tot_info = np.concatenate(np_imvecs, axis = 0)
            tot_info = np.concatenate((tot_info,  self.last_known_water_levels, self.elapsed_time_from_lkwl))
            local_obs.append(tot_info)
            shared_obs.append(tot_info)
            available_actions.append(self.agt_avail_acts[0])
        else:
            for i, obs_img in enumerate(rgb_imgs):
                torch_im = torch.from_numpy(np.transpose(obs_img, (2,0,1)).copy()).float()
                torch_imvec = resnet(torch_im.unsqueeze(0))  # return (1, 512)
        ...
        # print("reset function; local obs: {}; shared obs: {}; avail acts: {}".format(local_obs, shared_obs, available_actions))
    ...
    def step(self, acts):
        ...
        while not np.any(isActsFinished):
            ...
            if self.args.mode == "single":
                ...
                if self.args.conditioned and self.args.ac_sp == "combi" and self.args.mode=="multi":
                    str_acts = [] # when use joint action space
                    for idx in range(self.num_agents):
                        str_acts.append(self.agent_types[idx] + "_" + self.action_list[acts[0][0]].split("_")[idx])
                else:
                    str_acts = []
...
#                rew_wat += -1.0/(np.mean(self.g_water_levels)+1e-3) + 1
        ...
        # Outside of while loop, Update step counter. 
        self.cur_ep_steps += 1
        ...
        if self.args.conditioned and self.args.ac_sp == "combi" and self.args.mode=="multi":
            ...
            info[0]["act"] = acts[0][0]
            info[0]["basic_steps"] = self.basic_steps
            info[0]["act_finished"] = []
            info[0]["act_names"] = []
            for i in range(self.num_agents):
                info[0]["act_names"].append(self.action_list[acts[0][0]].split("_")[i])
                if isActsFinished[i]:
                    info[0]["act_finished"].append(True)
                else:
                    info[0]["act_finished"].append(False)
        ...
        # print("step function; local obs: {}; shared obs: {};".format(local_obs, shared_obs))
